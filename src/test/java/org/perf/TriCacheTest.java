...
public class TriCacheTest {
    ...
    public void testDataBatchAdd() {
        DataBatch<Integer,byte[]> batch=new DataBatch<>(null, 5);
        for(int i=0; i < 5; i++) {
            Data<Integer,byte[]> d=new Data<>(PUT, 322649+i, i, new byte[1024], null);
            batch.add(d);
        }
    ...
    public static Data<Integer,byte[]>[] create() {
        Data<Integer,byte[]>[] data=new Data[10];
