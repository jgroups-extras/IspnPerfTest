...
public class TestLocal extends ReceiverAdapter {
    ...
    static {
        try {
            METHODS[START_UPERF]  = TestLocal.class.getMethod("startUPerfTest");
            METHODS[START_ISPN]   = TestLocal.class.getMethod("startIspnTest");
            METHODS[GET]          = TestLocal.class.getMethod("get", long.class);
            METHODS[PUT]          = TestLocal.class.getMethod("put", long.class, byte[].class);
            METHODS[GET_CONFIG]   = TestLocal.class.getMethod("getConfig");
            METHODS[SET]          = TestLocal.class.getMethod("set", String.class, Object.class);
            METHODS[QUIT_ALL]     = TestLocal.class.getMethod("quitAll");
    ...
    public Config getConfig() {
        ...
        for(Field field: Util.getAllDeclaredFields(TestLocal.class)) {
            ...
            switch(c) {
                ...
                case 'v':
                    System.out.printf("JGroups: %s\n, Infinispan: %s\n",
                                      Version.printDescription(), org.infinispan.Version.printVersion());
                    break;
                case 'q':
                    stop();
                    return;
    ...
    protected void printCacheSize() {
        System.out.printf("-- cache has %d elements\n", cache.size());
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        TestLocal[] tests={new TestLocal(), new TestLocal(), new TestLocal(), new TestLocal()};
        try {
            int ch='A';
            for(TestLocal test: tests)
                test.init(cache_factory_name, config_file, jgroups_config, cache_name + ((char)ch++));
            tests[0].eventLoop();
        }
        catch(Throwable ex) {
            ex.printStackTrace();
